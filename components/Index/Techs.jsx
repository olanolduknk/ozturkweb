            ...
            <div className="gap-2 p-5 w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                {techs
                    ? techs.map((tech, index) => {
                          ...
                          // url yoksa kart tıklanmasın (bozulmasın)
                          ...
                          return (
                              <Wrapper
                                  ...
                                  {...wrapperProps}
                                  className={
                                      "bg-[#080808] hover:-translate-y-[3px] hover:bg-[#101010] transition-all duration-200 rounded-lg p-2 px-3 flex items-center justify-between " +
                                      (url ? "cursor-pointer" : "")
                                  }
                              >
                                  <img src={"/img/techs/" + icon} className="h-6 w-6" alt={name} />
                                  <span>{name}</span>
                              </Wrapper>
                          );
                      })
                    : Array.from({ length: 12 }).map((_, index) => (
                          ...
                          >
                              ...
                              <div
                                  style={{ width: `${[5, 6, 7][Math.floor(Math.random() * 3)]}rem` }}
                                  className="animate-pulse h-6 rounded-lg bg-white/10"
                              />
                          </div>
                      ))}
            </div>
