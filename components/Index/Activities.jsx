...
export default function Activities() {
  ...
  if (!data) {
    return (
      <div className="pt-5 pb-10 md:grid grid-cols-3 gap-x-6">
        ...
        <div className="mt-5 md:mt-0 rounded-lg overflow-hidden col-span-2 w-full h-[140px] py-5 relative bg-[#080808] flex items-center">
  ...
  // error / unavailable (but NOT crashing)
  if (!data.ok) {
    return (
      <div className="pt-5 pb-10 md:grid grid-cols-3 gap-x-6">
        <div className="flex items-center space-x-5 rounded-lg bg-[#080808] p-5">
          ...
          <div className="flex-1">
            ...
            <h1 className="text-center leading-none font-semibold text-sm text-zinc-400">
        ...
        <div className="mt-5 md:mt-0 rounded-lg overflow-hidden col-span-2 w-full h-[140px] py-5 relative bg-[#080808] flex items-center">
          <div className="w-full px-6 flex items-center justify-between">
            <div className="text-zinc-300">
              Spotify/Discord status şu an alınamadı.
            </div>
            <div className="text-zinc-600 text-xs uppercase">
              {data.error}{data.http_status ? ` (${data.http_status})` : ""}
            </div>
          </div>
        </div>
      </div>
  ...
  // ok
  const profile = data;
  const status = typeof profile.status === "string" ? profile.status : "offline";
          ...
          <Tippy interactive={true} content={statusUpper}>
            <div className="absolute bottom-1 bg-[#080808] rounded-full right-1 w-6 h-6 flex items-center justify-center">
              <div className="w-full h-full relative flex items-center justify-center">
                ...
                <div className={"absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-3 h-3 rounded-full " + `discord-${status}`} />
              </div>
            </div>
          </Tippy>
        </div>
